    ...
    for device in local.devices : [
      {
        ...
        pce_peers = [
          for peer in try(local.device_config[device.name].segment_routing_te.pce_peers, local.defaults.iosxr.configuration.segment_routing_te.pce_peers, []) : {
            ...
            precedence  = try(peer.precedence, null)
          }
        ]
        ...
        on_demand_colors = [
          for color in try(local.device_config[device.name].segment_routing_te.on_demand_colors, local.defaults.iosxr.configuration.segment_routing_te.on_demand_colors, []) : {
            ...
            source_address                      = try(color.source_address, null)
            source_address_type                 = try(color.source_address_type, null)
            constraint_segments_protection_type = try(color.constraint_segments_protection_type, null)
            constraint_segments_sid_algorithm   = try(color.constraint_segments_sid_algorithm, null)
            dynamic_anycast_sid_inclusion       = try(color.dynamic_anycast_sid_inclusion, null)
            dynamic_metric_type                 = try(color.dynamic_metric_type, null)
            effective_metric_enable             = try(color.effective_metric_enable, null)
            effective_metric_type               = try(color.effective_metric_type, null)
            effective_metric_value              = try(color.effective_metric_value, null)
            srv6_enable                         = try(color.srv6_enable, null)
            srv6_locator_behavior               = try(color.srv6_locator_behavior, null)
            srv6_locator_binding_sid_type       = try(color.srv6_locator_binding_sid_type, null)
            srv6_locator_name                   = try(color.srv6_locator_name, null)
          }
        ]
        ...
        policies = [
          for policy in try(local.device_config[device.name].segment_routing_te.policies, local.defaults.iosxr.configuration.segment_routing_te.policies, []) : {
            policy_name                   = try(policy.policy_name, null)
            source_address                = try(policy.source_address, null)
            source_address_type           = try(policy.source_address_type, null)
            policy_color_endpoint_address = try(policy.policy_color_endpoint_address, null)
            policy_color_endpoint_color   = try(policy.policy_color_endpoint_color, null)
            policy_color_endpoint_type    = try(policy.policy_color_endpoint_type, null)
            srv6_enable                   = try(policy.srv6_enable, null)
            srv6_locator_behavior         = try(policy.srv6_locator_behavior, null)
            srv6_locator_binding_sid_type = try(policy.srv6_locator_binding_sid_type, null)
            srv6_locator_name             = try(policy.srv6_locator_name, null)
          }
        ]
      }
